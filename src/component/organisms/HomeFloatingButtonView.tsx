...
const HomeFloatingButtonView = ({
    ...
    return (
        <Animated.View pointerEvents={isVisible ? 'auto' : 'none'} style={[styles.container, fadeStyle]}>
            <TouchableOpacity activeOpacity={1} onPress={onPressBack} style={styles.buttonView}>
                {buttonList?.map((element: any, index: number) => {
                    ...
                    if (showViews) {
                        transform.value = (-sizeConverter(32) - sizeConverter(12)) * (index + 1)
                    } else {
                        transform.value = 0
                    }
...
const StatusItem = ({ item, transformAnimatedStyles, containerStyle, onPress }: StatusItemProps) => {
    return (
