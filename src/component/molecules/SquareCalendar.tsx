        ...
        <View style={styles.container}>
            <View style={{ flexDirection: 'row' }}>
                {DAYS.map((item: any, index: number) => {
                    return (
                        <View style={{ width: buttonSize, alignItems: 'center' }} key={`${index}-keys`}>
                            <DateButton disabled textStyle={styles.dateTextStyle} text={item['id'] || item} />
                        </View>
                    )
                })}
            </View>
                ...
                return (
                    <View style={{
                        ...
                        {item?.map((item: any, index: number) => {
                            return (
                                <View key={`${index}-keys`}>
                                    <DateButton textStyle={{ ...styles.dateNumberTextStyle, color: item['month'] === 'cur' ? themeColor[theme].seegnal_dark_gray : themeColor[theme].seegnal_gray }} buttonStyle={styles.dateButtonStyle} text={item['day']} />
                                </View>
                            )
                        })}
                    </View>
