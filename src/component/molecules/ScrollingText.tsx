...
const ScrollingText = ({ animatedValue, data, sliderWidth, titleStyle, textStyle, style }: ScrollingTextProps) => {
    ...
    data.forEach((element, index) => {
        if (index === 0) {
            ...
            animArray.push(
                animatedValue.interpolate({
                    inputRange: [(index - 2) * sliderWidth, (index - 1) * sliderWidth, index * sliderWidth],
            ...
            animArray.push(
                animatedValue.interpolate({
                    inputRange: [(index - 1) * sliderWidth, index * sliderWidth, (index + 1) * sliderWidth],
    ...
    return (
        <View style={styles.textView}>
            {data.map((element, index) => {
                ...
                return (
                    <Animated.View key={`${index}_keys`} style={{ justifyContent: 'center', alignItems: 'center', position: 'absolute',opacity:opacity }}>
