...
const ImageList = ({ renderItem, data, ref, sliderWidth, itemWidth, animatedValue, containerStyle, listStyle, listContainerStyle }: ImageListProps) => {
    ...
    return (
        <View style={{ ...containerStyle }}>
            <Animated.FlatList
                ...
                horizontal
                ref={ref}
                keyExtractor={(item: any, index: number) => `${item?.id}}${index}_onboard`}
                renderItem={renderItem}
                data={data}
                onScroll={Animated.event(
                    [
                        { nativeEvent: { contentOffset: { x: animatedValue || caroselAnimatedValue.current } } }
                    ],
                    {
